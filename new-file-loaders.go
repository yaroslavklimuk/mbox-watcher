...
func LoadNewFilesFromTextFile(c Config, stopChannel chan bool) chan NewFileToWatch {
	...
	go func() {
		...
		watcher.Add(c.NewFilesSourceTextfile)
			...
			case event, ok := <-watcher.Events:
				if ok {
					if event.Op == fsnotify.Write {
						jsonContents, _ := ioutil.ReadFile(c.NewFilesSourceTextfile)
